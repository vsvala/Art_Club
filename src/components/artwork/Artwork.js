...
const Artwork = ({ artwork, onClick, notify, voteArtwork }) => {
  const addLike = (artwork) => {
    ...
    return () => {
      ...
      notify(
        `Artworkn ${artworkObject.title} by ${artwork.artist} like added`,
        5,
      );
      //   })
      //   .catch(error => {
    ...
    <div className="artworkGallery">
      ...
      <ul style="list-style-type:none;">
        ...
        <li className="artwork">
          ...
          by {artwork.artist}, {artwork.year}, {artwork.size}, {artwork.medium}
        </li>
        ...
        <p>
          {artwork.likes}likes <button onClick={addLike(artwork)}>like</button>
        </p>
        <p>Added by {artwork.user.name}</p>
        <li className="delete">
          <DeleteButton id={artwork.id} onClick={onClick} />
          TODO DELETE hidden vain adminille
        </li>
      </ul>
    </div>
  );
};
export default connect(null, { notify, voteArtwork })(Artwork);
